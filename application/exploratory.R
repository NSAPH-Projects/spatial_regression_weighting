...
gCAR <- ggplot() +
  ...
  scale_color_gradient2(
    low = "forestgreen", mid = "white", high = "darkred", midpoint = 0, 
  ...
  scale_color_gradient2(
    low = "darkred", mid = "white", high = "forestgreen", midpoint = 0, 
...
iw <- compute_impliedweights(X = as.matrix(X[,1:11]), 
                             Z = Z, 
                             clusters = clusters, 
                             adj = adjacency_matrix, 
                             sig2gam = 0.01, 
                             phi = 0.8)
sum(iw$Pooled[Z == 1]);sum(iw$Pooled[Z == 0])
sum(iw$FE[Z == 1]);sum(iw$FE[Z == 0])
sum(iw$RE[Z == 1]);sum(iw$RE[Z == 0])
sum(iw$GP[Z == 1]);sum(iw$GP[Z == 0])
sum(iw$CAR[Z == 1]);sum(iw$CAR[Z == 0])
sum(iw$SAR[Z == 1]);sum(iw$SAR[Z == 0])

allmint <- min(0,min(c(iw$CAR[Z == 1], 
                       iw$FE[Z == 1],
                       iw$GP[Z == 1])))
allmaxt <- max(c(iw$CAR[Z == 1], 
                  iw$FE[Z == 1],
                  iw$GP[Z == 1]))
allminc <- min(0,min(c(iw$CAR[Z == 0], 
                       iw$FE[Z == 0],
                       iw$GP[Z == 0])))
allmaxc <- max(c(iw$CAR[Z == 0], 
                  iw$FE[Z == 0],
                  iw$GP[Z == 0]))

# Merge the base weights with the data
buffer_centroids <- st_centroid(buffers)
buffers_merged <- cbind(buffer_centroids, iw)

# Plot baseCAR
buffers_merged <- buffers_merged[order(buffers_merged$CAR),]
gCAR <- ggplot() +
  geom_sf(data = states, fill = "black", color = "white", linetype = "solid") +
  geom_sf(data = buffers_merged[buffers_merged$Z == 0,], aes(color = CAR, shape = 'Control'), size = 1.5) +
  labs(
    shape = "Treatment",
    color = "Implied weights (control)"
  ) +
  scale_color_gradient2(
    low = "forestgreen", mid = "white", high = "darkred", midpoint = 0, 
    limits = c(allminc, allmaxc)) + #limits = c(0, max(buffers_merged$baseCAR[buffers_merged$Z == 0]))) + 
  new_scale_color() +
  geom_sf(data = buffers_merged[buffers_merged$Z == 1,], aes(color = CAR, shape = 'Treated'), size = 1.5) +
  scale_color_gradient2(
    low = "darkred", mid = "white", high = "forestgreen", midpoint = 0, 
    limits = c(allmint, allmaxt)) + #c(0, max(buffers_merged$CAR[buffers_merged$Z == 1]))) + 
  coord_sf(xlim = c(-125, -65), ylim = c(25, 50), expand = FALSE) +
  labs(
    title = "Conditional Autoregressive",
    x = "Longitude",
...
# Plot  FE
buffers_merged <- buffers_merged[order(buffers_merged$FE),]
gRE <- ggplot() +
  geom_sf(data = states, fill = "black", color = "white", linetype = "solid") +
  geom_sf(data = buffers_merged[buffers_merged$Z == 0,], aes(color = FE, shape = 'Control'), size = 1.5) +
  labs(
    shape = "Treatment",
    color = "Implied weights (control)"
  ) +
  scale_color_gradient2(
    low = "forestgreen", mid = "white", high = "darkred", midpoint = 0, 
    limits = c(allminc, allmaxc)) + #limits = c(0, max(buffers_merged$FE[buffers_merged$Z == 0]))) + 
  new_scale_color() +
  geom_sf(data = buffers_merged[buffers_merged$Z == 1,], aes(color = FE, shape = 'Treated'), size = 1.5) +
  scale_color_gradient2(
    low = "darkred", mid = "white", high = "forestgreen", midpoint = 0, 
    limits = c(allmint, allmaxt)) + #limits = c(0, max(buffers_merged$FE[buffers_merged$Z == 1]))) + 
  coord_sf(xlim = c(-125, -65), ylim = c(25, 50), expand = FALSE) +
  labs(
    title = "Random Effects",
    x = "Longitude",
...
# Plot  GP
buffers_merged <- buffers_merged[order(buffers_merged$GP),]
gGP <- ggplot() +
  geom_sf(data = states, fill = "black", color = "white", linetype = "solid") +
  geom_sf(data = buffers_merged[buffers_merged$Z == 0,], aes(color = GP, shape = 'Control'), size = 1.5) +
  labs(
    shape = "Treatment",
    color = "Implied weights (control)"
  ) +
  scale_color_gradient2(
    low = "forestgreen", mid = "white", high = "darkred", midpoint = 0, 
    limits = c(allminc, allmaxc)) +#limits = c(0, 
  #          max(buffers_merged$GP[buffers_merged$Z == 0]))) + 
  new_scale_color() +
  geom_sf(data = buffers_merged[buffers_merged$Z == 1,], aes(color = GP, shape = 'Treated'), size = 1.5) +
  scale_color_gradient2(
    low = "darkred", mid = "white", high = "forestgreen", midpoint = 0, 
    limits = c(allmint, allmaxt)) +  #limits = c(0,
  #max(buffers_merged$GP[buffers_merged$Z == 1]))) + 
  coord_sf(xlim = c(-125, -65), ylim = c(25, 50), expand = FALSE) +
  labs(
    title = "Gaussian Process",
    x = "Longitude",
...
png('images/impliedweights.png', width = 1800, height = 2200, res = 200)
print(combined_plot)
#grid.arrange(gRE, gCAR, gGP, ncol = 1)
dev.off()
