...
# PLOT weights x OUTCOMES 
# buffers_merged_geo <- buffers_merged_geo[order(buffers_merged_geo$Y*buffers_merged_geo$SWiw),]
# gwY <- ggplot() +
#   scale_shape_manual(values = c("0" = 21, "1" = 24), guide  = guide_legend(order = 1)) +
#   geom_sf(
#     data   = states[states$STATEFP10 %in% c("34", "42"), ],
#     fill   = NA,
#     colour = "darkgray",
#     size   = .5
#   ) +
#   # Plot both treated and control sics in viridis
#   geom_sf(
#     data  = subset(buffers_merged_geo, Z == 0),
#     aes(fill = 100*SWiw*Y, shape = factor(Z)),
#     size   = 2,
#     stroke = .25,
#     colour = "black"
#   ) +
...
#     labels     = function(x)
#       sprintf("%.3f", x),
#     name   = expression("Control " * w[i] * Y[i]),   # ← legend title
#     guide      = guide_colorbar(order = 2, barheight = unit(1.0, "cm"))   # <<--- taller
#   ) +
#   
#   new_scale_fill() +
...
#     labels     = function(x)
#       sprintf("%.3f", x),
#     name   = expression("Treated " * w[i] * Y[i]),   # ← legend title
#     guide      = guide_colorbar(order = 3, barheight = unit(1, "cm"))  # <<--- shorter
#   ) +
#   labs(title = "Weights x Outcomes", shape = "Treatment") +
#   theme_minimal() +
#   theme(
#     panel.grid   = element_blank(),
#     axis.text    = element_blank(),
...
#     legend.position = "right"
#   )
# png('images/spatial_weightsoutomes_map.png',
#     width = 1600,
#     height = 1000,
#     res = 250)
# print(gwY)
...
# NOW calculate the sics
# sics <- sample_influence(Y = 100*buffers_outcome$Y,
#                          X,
#                          Z = buffers_outcome$Z,
#                          Vre = Vre,
#                          Vcar = Vcar,
...
# 
# buffer_centroids <- st_centroid(buffers_outcome)
# buffers_merged <- cbind(buffer_centroids, sics)
# 
# buffers_merged_geo <- st_transform(buffers_merged, crs = 4326)
# 
# buffers_merged_geo <- buffers_merged_geo[order(abs(buffers_merged_geo$sics)),]
# gsics <- ggplot() +
#   scale_shape_manual(values = c("0" = 21, "1" = 24), guide  = guide_legend(order = 1)) +
#   geom_sf(
#     data   = states[states$STATEFP10 %in% c("34", "42"), ],
...
#   geom_sf(
#     data = buffers_merged_geo,
#     aes(fill = sics, shape = factor(buffers_merged_geo$Z)),
#     size   = 2,
#     stroke = .25,
#     colour = "black"
#   ) +
...
#     guide     = guide_colorbar(order = 2, barheight = unit(1.0, "cm"))
#   ) +
#   theme_minimal() +
#   theme(
#     panel.grid   = element_blank(),
