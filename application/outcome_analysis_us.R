...
############################ Fit methods ###############################
# https://www.census.gov/cgi-bin/geo/shapefiles/index.php?year=2016&layergroup=ZIP+Code+Tabulation+Areas
zctas <- st_read('../shapefiles/tl_2020_us_zcta520/tl_2020_us_zcta520.shp') # 33144
zctas <- st_transform(zctas, 5070)
zctas$zctas_id <- 1:nrow(zctas)
# For now, use synthetic data
zctas$Lw_Brth <- rpois(nrow(zctas), lambda = 20) # Low birth weight births
zctas$Nm_Br_L <- rpois(nrow(zctas), lambda = 1000) # Number of live births
zctas$Ttl_Brt <- rpois(nrow(zctas), lambda = 150000) # Total birth weight (grams)
zctas$Nm_Br_B <- rpois(nrow(zctas), lambda = 1000) # Number of births
zctas$Infnt_M <- rpois(nrow(zctas), lambda = 5) # Infant mortality
zctas$Nm_Br_I <- rpois(nrow(zctas), lambda = 1000) # Number of infant births
zctas$Preterm <- rpois(nrow(zctas), lambda = 100) # Preterm births
zctas$Nm_Br_P <- rpois(nrow(zctas), lambda = 1000) # Number of births
zctas$Nm_Br_A <- rpois(nrow(zctas), lambda = 1000) # Number of births
zctas$Avg_G_A <- rnorm(nrow(zctas), mean = 39, sd = 2) * zctas$Nm_Br_A # Average gestational age (weeks)

sf_use_s2(FALSE)
# Compute intersections between buffers and ZCTAs
# Use spatial index to keep only zctas that actually intersect any buffer
buf_u   <- st_union(st_geometry(buffers))   # cheaper to test against one unioned shape
hits    <- st_intersects(zctas, buf_u)   
zctas_sub <- zctas[lengths(hits) > 0, , drop = FALSE]
# Select only the birth columns needed for analysis
zctas_sub <- zctas_sub %>% 
  dplyr::select(
    zctas_id,
    Lw_Brth,
...
start_time <- Sys.time()
intersections <- st_intersection(zctas_sub, buffers)
print(Sys.time() - start_time) # Time the intersection operation

# Compute area of each intersection
intersections <- intersections %>% 
  mutate(int_area = as.numeric(st_area(.)))

# Assume each ZCTA has a total area field (zctas_area)
# If not, compute it:
zctas <- zctas %>% mutate(zctas_area = as.numeric(st_area(.)))

# Join ZCTA total area to intersections (if not already present)
# Here we assume an identifier 'zctas_id' exists in both data frames
intersections <- intersections %>% 
  left_join(zctas %>% st_set_geometry(NULL) %>% dplyr::select(zctas_id, zctas_area), by = "zctas_id")

# Calculate the area fraction for each intersection
intersections <- intersections %>% 
  mutate(area_fraction = int_area / zctas_area)
